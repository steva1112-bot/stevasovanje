...
window.egipatSetup=function(){
  ...
  for(var i=0;i<pages.length;i++){
    ...
    pages[i].forEach(function(t){var p=document.createElement('p');p.className='book-page-text';p.textContent=t;sl.appendChild(p);});
    cont.appendChild(sl);
  }
  ...
  window.egipatNext=function(){if(n+2<tot)sh(n+2);};window.egipatPrev=function(){if(n>=2)sh(n-2);};sh(0);
};
