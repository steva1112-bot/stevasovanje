...
window.egipatSetup=function(){
  ...
  var pages=bpE(P);cont.innerHTML='';
  for(var i=0;i<pages.length;i++){
  ...
  function sh(i){
    n=i;
    Array.from(cont.children).forEach(function(s,j){s.classList.remove('active','right-visible');if(j===i)s.classList.add('active');if(j===i+1)s.classList.add('right-visible');});
    if(co)co.textContent=(Math.floor(i/2)+1)+' / '+pairTot;
    if(pv)pv.disabled=(i===0);if(nx)nx.disabled=(i+2>=tot);
  }
